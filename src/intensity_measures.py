...
'''
Obtained & modified from https://github.com/GEMScienceTools/gmpe-smtk

Tools for supplementing Ground Motion Intensity Measures (IMs) from a set of
...
def rotdpp(acceleration_x, time_step_x, acceleration_y, time_step_y, periods,
    ...
    max_a_theta = np.zeros((len(theta_set), len(periods) + 1), dtype=np.float32)
